...
(function () {
    ...
    function buildPersistentAudio() {
        ...
        try {
            const savedVol = localStorage.getItem('felicia_volume');
            if (savedVol !== null) globalAudio.volume = parseFloat(savedVol);
        } catch(_){}
    ...
    function reinit() {
        ...
        try { if(typeof startWIBClock === 'function') startWIBClock(); } catch(_){}
    ...
    function wireDashboardPlayer() {
        ...
        if (volSlider) {
            if (globalAudio) volSlider.value = globalAudio.volume;
            volSlider.oninput = () => {
                if (globalAudio) {
                    globalAudio.volume = parseFloat(volSlider.value);
                    try { localStorage.setItem('felicia_volume', volSlider.value); } catch(_){}
                }
            };
        }
    ...
    // =====================================================================
    // § X — WIB CLOCK & SCHEDULE HIGHLIGHT
    // =====================================================================
    window.startWIBClock = function() {
        const timeEl = document.getElementById('wib-clock-time');
        const dateEl = document.getElementById('wib-clock-date');
        
        function update() {
            const now = new Date();
            // Get time in WIB (Asia/Jakarta)
            const formatterTime = new Intl.DateTimeFormat('id-ID', { timeZone: 'Asia/Jakarta', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false });
            const formatterDate = new Intl.DateTimeFormat('id-ID', { timeZone: 'Asia/Jakarta', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
            
            if (timeEl) timeEl.innerText = formatterTime.format(now).replace(/\./g, ':');
            if (dateEl) dateEl.innerText = formatterDate.format(now);

            // Highlight schedule based on WIB day
            // id-ID weekday output format can vary, so let's use en-US to accurately identify the day
            const dayParts = new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Jakarta', weekday: 'short' }).formatToParts(now);
            const wibDayStr = dayParts.find(p => p.type === 'weekday')?.value;
            let dayIndex = 0;
            if (wibDayStr === 'Mon') dayIndex = 1;
            else if (wibDayStr === 'Tue') dayIndex = 2;
            else if (wibDayStr === 'Wed') dayIndex = 3;
            else if (wibDayStr === 'Thu') dayIndex = 4;
            else if (wibDayStr === 'Fri') dayIndex = 5;
            ...
            for (let i = 1; i <= 5; i++) {
                ...
                if (el) {
                    if (i === dayIndex) {
                        el.classList.add('ring-4', 'ring-cyan-400', 'shadow-[0_0_20px_rgba(34,211,238,0.5)]', 'scale-[1.02]', 'z-10');
                    } else {
                        el.classList.remove('ring-4', 'ring-cyan-400', 'shadow-[0_0_20px_rgba(34,211,238,0.5)]', 'scale-[1.02]', 'z-10');
                    }
                }
            }
        }
        ...
        update();
        if (window._wibClockInterval) clearInterval(window._wibClockInterval);
        window._wibClockInterval = setInterval(update, 1000);
    }
